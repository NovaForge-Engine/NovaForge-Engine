...
PhysicsEngine::PhysicsEngine()
	: tempAllocator(nullptr), jobSystem(nullptr), debugRenderer(nullptr),
	  collisionMatrix(collisionLayersCount,
                      std::vector<bool>(collisionLayersCount, false))
{
}
...
void PhysicsEngine::Init(float gravityScale)
{
	...
	physicsSystem.SetGravity(
		JPH::Vec3(0.0f, -9.81f / 100 * gravityScale, 0.0f));
...
bool PhysicsEngine::CanCollide(CollisionLayer layer1,
                               CollisionLayer layer2) const
{
	auto l1 = ToJoltLayer(layer1);
	auto l2 = ToJoltLayer(layer2);
	JPH_ASSERT(l1 < collisionMatrix.size() && l2 < collisionMatrix[l1].size(),
	           "Trying to compare wrong collision layers");

	return collisionMatrix[l1][l2];
}

void PhysicsEngine::SetCollisionRule(CollisionLayer layer1,
                                     CollisionLayer layer2, bool collisionRule)
{
	auto l1 = ToJoltLayer(layer1);
	auto l2 = ToJoltLayer(layer2);
	JPH_ASSERT(l1 < collisionMatrix.size() && l2 < collisionMatrix[l1].size(),
	           "Trying to set collision rule for wrong collision layers");
...
{
	JPH_ASSERT(novaToJoltBodyIdTable.contains(novaBodyId),
	           "Trying get JoltBodyID with invalid NovaBodyID");
	return novaToJoltBodyIdTable.at(novaBodyId);
}

NovaBodyID PhysicsEngine::GetBodyID(JoltBodyID joltBodyId)
{
	JPH_ASSERT(joltToNovaBodyIdTable.contains(joltBodyId),
	           "Trying get NovaBodyID with invalid JoltBodyID");
	return joltToNovaBodyIdTable.at(joltBodyId);
}

NovaBodyID PhysicsEngine::CreateBody(JPH::BodyCreationSettings& inSettings,
                                     float mass)
{
...
//
// ID PhysicsEngine::CreateBody(JPH::ShapeSettings* inShapeSettings,
// JPH::RVec3Arg inPosition, JPH::QuatArg inRotation, JPH::EMotionType
// inMotionType, JPH::ObjectLayer collisionLayer, float mass)
//{
//	return CreateBody(JPH::BodyCreationSettings(inShapeSettings, inPosition,
//inRotation, inMotionType, collisionLayer), mass);
//}
//
//
// ID PhysicsEngine::CreateBody(JPH::Shape* inShape, JPH::RVec3Arg inPosition,
// JPH::QuatArg inRotation, JPH::EMotionType inMotionType, JPH::ObjectLayer
// collisionLayer, float mass)
//{
//	return CreateBody(JPH::BodyCreationSettings(inShape, inPosition, inRotation,
//inMotionType, collisionLayer), mass);
//}

//(@Tenzy21 | 04.01.2025) TODO: Separate adding and creating logic + Implement
//other signatures later
NovaBodyID PhysicsEngine::CreateAndAddBody(const JPH::Shape* shape,
                                           ...
                                           float mass)
{
	...
	JPH::BodyCreationSettings bodySettings(shape, position, rotation,
	                                       motionType, collisionLayer);
	JPH::MassProperties msp;
	msp.ScaleToMass(mass);
...
void PhysicsEngine::RemoveBody(NovaBodyID id)
{
	...
	{
		spdlog::warn(
			"Jolt | RemoveBody: Trying to remove non-existed body (ID: {})",
			id);
	}
}
void PhysicsEngine::DestroyBody(NovaBodyID id)
{
	...
	{
		spdlog::warn(
			"Jolt | DestroyBody: Trying to destroy non-existed body (ID: {})",
			id);
	}
}
...
void PhysicsEngine::SetBodyTransform(NovaBodyID id, const JPH::Vec3& position,
                                     const JPH::Quat& rotation)
{
	JoltBodyID bodyID = GetBodyID(id);
	bodyInterface.SetPositionAndRotation(bodyID, position, rotation,
